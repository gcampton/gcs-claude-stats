...
Fork of https://github.com/aiedwardyi/claude-usage-monitor (MIT).
Modifications: context bar fills as USED (not remaining); 7d reset time
always shown; session duration prefixed with ⏱ glyph.
...
# CUSTOM: bar fills as USED (consistent with 5h/7d)
...
ctx_str = f"{ctx_color}{gauge}{N} {ctx_pct_used}%"  # CUSTOM: was ctx_remaining
...
# Duration — CUSTOM: prefixed with ⏱ so it's not mistaken for the 7d reset time
if SHOW_DURATION:
